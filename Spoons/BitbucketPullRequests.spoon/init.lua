...
function getPRs(username, password)
    ...
    colorWhite = {red = 255, blue = 255, green = 255}
    doMenu()
...
function parseBBJson(username, password)
    ...
    if status == 200 then
        ...
        for key, value in pairs(bodyTable.values) do
            ...
            local url2 = value.links.self.href:gsub("%!", "")
            ...
            if status2 == 200 then
                ...
                print('bb call2 failed with status code: ' .. status2)
            end
        end
    else
...
function addPRsToMenu(allPRs)
    ...
    for key, value in pairs(allPRs) do
        ...
        if BBprev ~= nil and BBprev[bbkey] ~= nil then
            value.prev = {
...
function doMenu()
    ...
    if num_unapproved == nil then
        ...
        menuItem:setTitle(
            hs.styledtext.new(
                num_unapproved,
                {color = colorWhite, superscript = 1, baselineOffset = -6, paragraphStyle = {alignment = 'right'}}
            ) ..
                hs.styledtext.new('/', {color = colorWhite, superscript = 0, baselineOffset = -3}) ..
                    hs.styledtext.new(
                        num_prs,
                        {
                            color = colorWhite,
